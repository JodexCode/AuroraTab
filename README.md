...
- **多语言支持**：新增 i18n 系统，支持英文和中文，默认自动检测设备语言
- **搜索引擎记忆**：选择的搜索引擎会在关闭标签页后自动保存
- **GitHub Actions CI**：新增自动化测试工作流

### 🔄 重构优化

- **面板系统重构**：工具栏从单一设置按钮重构为四个独立入口（外观、语言、设置、壁纸）
- **目录结构优化**：组件迁移到 `src/components/panels/`，新标签页独立为 `src/newtab/`
- **NumberInput 组件**：自定义数字输入框，替换原生组件
